...
function WeLiveWin() {
	this.Create = function(id, title, wbody, lang, au){
		...
		g_tools.children(".t_smilies").tipTip({content: $(".smilies_div").html().replace(/towhere/ig, id), keepAlive:true, maxWidth:"348px", defaultPosition:"top", edgeOffset:0, delay:300, parent: true, hoverClass: "hover"});

		g_tools.children(".t_phrase").tipTip({content: (lang == 1 ? $(".phrases_div").html().replace(/towhere/ig, id) : $(".phrasesen_div").html().replace(/towhere/ig, id)), keepAlive:true, maxWidth:"348px", defaultPosition:"top", edgeOffset:0, delay:400, parent: true, hoverClass: "hover"});
...
function welive_parseOut(data){
	...
	switch(data.x){
		...
		case 6: //客服特别操作及反馈信息
			switch(data.a){
				case 8: //客人登录成功
					guest_create(data.g, data.n, data.l, data.re, parseInt(data.au), parseInt(data.mb)); //创建新客人, 在创建中输出信息
...
function guest_create(gid, n, lang, old, au, mb){
	if(!gid) return;
	...
	if(i > -1){ //表示客人重新上线
		...
		g_online.prepend('<div id="g' + gid + '" onclick="openWin(' + gid + ');" class="g" title="' + n + '"><i>' + n + '</i><b>0</b>' + ((mb == 1)? '<u></u>'  : '') + '</div>');
...
function guest_delete(gid){
	$('#win'+gid).remove(); //清除窗口
	g_online.find('#g' + gid).remove(); //更新列表
...
var myWin88, CurrentId = 0, zIndex = 2000;
