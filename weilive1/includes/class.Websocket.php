...
class Websocket{
	...
	**/
						...
						if(!$this->is_robot){ //非无人值守时
							foreach($this->guest AS $k => $g){
								if($g['aid'] == $aid){
									...
									$guest_list[] = array('g' => $g['gid'], 'n' => $g['n'], 'l' => $g['l'], 'au' => $g['au'], 'mb' => $g['mb']); //记录仍然在线的属于自己的客人, au指上传授权
								}
							}
						}
			...
			case 6: //客人特别操作及反馈信息
				...
				switch($data['a']){
					...
					case 8: //客人连接验证及分配给客服
						...
						$fullname = decodeChar($data['fn']); //客人姓名
						...
						if($first AND $gid AND $hasRecord){  //首次连接, 有gid, 且在数据库中有记录时, 更新存在的客人信息 === 即老客人第一次登录, 非重连
							...
							APP::$DB->exe("UPDATE " . TABLE_PREFIX . "guest SET aid = '$aid', lang ='$lang', logins = (logins + 1), last = '$timenow', lastip = '$lastip', ipzone = '$ipzone', browser = '$browser', mobile = '$mobile', fromurl = '$fromurl', session = '$session' WHERE gid = '$gid'");
							...
							APP::$DB->exe("INSERT INTO " . TABLE_PREFIX . "guest (aid, upload, lang, last, lastip, ipzone, browser, mobile, fromurl, remark, session)
									VALUES ('$aid', '0', '$lang', '$timenow', '$lastip', '$ipzone', '$browser', '$mobile', '$fromurl', '', '$session')");
						...
						//记录客人信息
						$this->guest[$index] = array('gid' => $gid, 'aid' => $aid, 'aix' => $admin_index, 'n' => $fullname, 'l' => $lang, 'mb' => $mobile, 'au' => $authupload);

						//给客服发送通知: 客人上线
						$this->send(array('x' => 6, 'a' => 8, 'g' => $gid, 'au' => $authupload, 'n' => $fullname, 'l' => $lang, 'mb' => $mobile, 're' => $recs), $this->accept[$admin_index], $admin_index);
	...
	function tranferGuest($data, $accept, $index, $guest_query = 0){
		...
		$this->send(array('x' => 6, 'a' => 8, 'g' => $gid, 'n' => $this->guest[$gix]['n'], 'au' => $this->guest[$gix]['au'], 'l' => $this->guest[$gix]['l'], 'mb' => $this->guest[$gix]['mb'], 're' => $recs), $this->accept[$aix], $aix);
